...
def check_rate_limit(student_name: str, max_per_day: int = DEFAULT_MAX_PER_DAY) -> None:
    """
    ...
    # Prune old entries
    timestamps = _requests[student_name]
    _requests[student_name] = [t for t in timestamps if t > cutoff]

    if len(_requests[student_name]) >= max_per_day:
        raise AppError(
            f"Daily limit reached ({max_per_day} requests/day). Try again tomorrow.",
            status_code=429,
        )

    _requests[student_name].append(now)
